...
from modules.ppo.models_ngo_SIMP import MaskablePPOPolicy_EMB_LSTM_SIMP
...
class TrajectoryDataset_EMB():
    ...
    def __next__(self):
        if self.batch_count * self.batch_size >= math.ceil(self.cumsum_seq_len[-1] / self.batch_len):
            ...
            return TrajectoryBatch_EMB(**{key: value[eps_idx, series_idx]for key, value
                                     in self.trajectories.items() if key in TrajectoryBatch_EMB.__dataclass_fields__.keys()},
...
def save_checkpoint(ppo_model, ppo_optimizer, iteration, stop_conditions, hp, tp):
    ...
    with open(CHECKPOINT_PATH + "ppo_model_class.pt", "wb") as f:
        pickle.dump(MaskablePPOPolicy_EMB_LSTM_SIMP, f)
...
def start_or_resume_from_checkpoint(env, config, hp, tp):
    ...
    if config['qnet']!='lin': assert False, "not implemented"
    ppo_algo = MaskablePPOPolicy_EMB_LSTM_SIMP
...
def gather_trajectories_EMB_SIMP(input_data,hp):
    """
    ...
    with torch.no_grad():
        ...
        for i in range(hp.rollout_steps):
            ...
            if first_pass: # initialize hidden states
                ppo_model.LSTM.reset_init_state(batch_size, gather_device)
                first_pass = False
                ...
                ppo_model.LSTM.hidden_cell[0][:,terminal.bool(),:] = 0.
                ppo_model.LSTM.hidden_cell[1][:,terminal.bool(),:] = 0.
            ...
            # Choose next action 
            features_mem = ppo_model.LSTM(features,terminal.to(gather_device))

            reachable=[]
            for e in env.envs:
                reachable.append(e.action_masks())
            reachable=torch.tensor(reachable,dtype=torch.bool).unsqueeze(0)
            action_dist, value = ppo_model(features_mem, reachable, terminal.to(gather_device))
...
def train_model_EMB_SIMP(env, ppo_model, ppo_optimizer, iteration, stop_conditions, hp, tp):   
    ...
    while iteration < stop_conditions.max_iterations:      
        ...
        trajectory_tensors = gather_trajectories_EMB_SIMP(input_data, hp)
        ...
        for epoch_idx in range(hp.ppo_epochs): 
            for batch in trajectory_dataset:
                ...
                features=ppo_model.LSTM(features)
                action_dist, values = ppo_model(features, None)
                ...
                #critic_loss = F.smooth_l1_loss(batch.discounted_returns, values.squeeze(-1))
...
def ConstructTrainSet(config, apply_wrappers=True, type_obs_wrap='Flat', remove_paths=False, tset='M3M5Mix'):
    global_env=[]
    probs=[]
    env_all_list=[]
    #config['solve_select'] = 'solvable'
    ...
    state_enc='nfm'
    edge_blocking = True
    env = GetCustomWorld('MemoryTaskU1',make_reflexive=True, state_repr=state_repr, state_enc=state_enc)
    nfm_func = modules.gnn.nfm_gen.nfm_funcs['NFM_ev_ec_t_dt_at_ustack']
    nfm_func.reinit(k=1) # set stack size
